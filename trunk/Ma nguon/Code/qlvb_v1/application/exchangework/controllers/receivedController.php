<?php
class exchangework_receivedController extends  Zend_Controller_Action {
	...
	public function init(){		
		...
		$this->view->currentModulCodeForLeft ="SUB_MENU_RECEIVED";
		/* Ket thuc*/
	...
	public function indexAction(){	
		...
		$this->view->bodyTitle = "DANH SÁCH CÔNG VIỆC ĐẾN";
		...
		//echo $sStaffRole;
		$arrResul = $objReceive->TaskWorkGetAll('CONG_VIEC_DEN',$_SESSION['staff_id'],$sStaffRole,$sUnitID,$sUnitType,'', $dFromDate, $dToDate, trim($sFullTextSearch),$checkstatus,$filetatus, $iCurrentPage, $iNumRowOnPage);
	...
	public function checkAction(){		
		...
		if($object_check_id != ''){
			$arrResul = $objReceive->TaskWorkRecCheck($_SESSION['staff_id'],$object_check_id);
		}
		$this->_redirect('exchangework/received/index/');
	}
	/**
	 * 
	...
	public function forwardAction(){		
		...
		//Cap nhat trang thai xem thong tin
		$arrReceived = $objReceive->TaskWorkStatusUpdate($_SESSION['staff_id'],$sTaskWorkId);
	...
	public function feedbackAction(){		
		...
		if ($this->_request->getParam('C_RESULT','')!= ""){		
			...
			$_SESSION['seArrParameter'] = $arrParaSet;
			$this->_request->setParams($arrParaSet);	
			$this->_redirect('exchangework/received/feedback/?hdn_object_id='.$sTaskWorkId);	
		}
	}
	/**
	...
	public function deletefeedbackAction(){
		...
		if ($sPkDocWorkIdList != ""){
			...
			$this->_redirect('exchangework/received/feedback/.?hdn_object_id='.$sReceiveDocumentId);	
		}	
	}
}?>