...
// Dinh nghia duong dan den thu vien cua Zend
	...
	$sRecordArchivedId	= $_REQUEST['RecordArchivedId'];
	$arrresult = $objRecordArchive->DocRecordArchiveDocRelateGetAll($sRecordArchivedId,'',$iCurentPage,$iRowOnPage,'','');
	//echo 'okokok';
	//var_dump($arrresult);
	$html = '<div style="border-bottom:1px solid black;margin-left:1%;width:98%;">';
	$html .= '<table cellpadding="0" cellspacing="0" border="0" width="100%" align="center" class="list_table2" id="table1">';
	$html .= "<col width='4%'><col width='5%'><col width='15%'><col width='15%'><col width='40%'><col width='20%'>";
				$v_current_style_name = "round_row";
				for($index = 0;$index < sizeof($arrresult);$index++){
					//Lay file dinh kem
					$documentId					= $arrresult[$index]['PK_DOC'];
					$strFileName 				= $arrresult[$index]['C_FILE_NAME'];
					$sFile 						= Sys_Publib_Library::_getAllFileAttach($strFileName,"!#~$|*","!~!","/../../../..".Sys_Init_Config::_setWebSitePath()."public/attach-file/");
					$sReleaseDate 				= $arrresult[$index]['C_RELEASE_DATE'].'&nbsp;';
					$sSymbol 					= $arrresult[$index]['C_SYMBOL'].'&nbsp;';
					$iStt = (($iCurentPage - 1) * $iRowOnPage) + $index;
					if($strFileName == '' || $strFileName == null){
						$sSubject				= $arrresult[$index]['C_SUBJECT'] .'&nbsp;';	
					}else{
						$sSubject				= $arrresult[$index]['C_SUBJECT'] .'&nbsp;' .'<br>'. $sFile;
						$sFile 					= "";
					}
					$sAgentcyGroup				= $arrresult[$index]['C_AGENTCY_GROUP'];
					$sAgentcyName				= $arrresult[$index]['C_AGENTCY_NAME'];
					if($sAgentcyGroup == '' or $sAgentcyName == '')
						$sReleasePlace 		    = $sAgentcyGroup.$sAgentcyName.'&nbsp;';
					else 
						$sReleasePlace			= $sAgentcyGroup.' -> '.$sAgentcyName;
					$sReleasePlace				= $arrresult[$index]['C_RELEASE_PLACE'];
					...
					$html .='<input type="checkbox" id="chk_item_id" name="chk_item_id" onclick ="selectrow_record(this,\''.$arrresult[$index]['C_DOC_TYPE'].'\',document.getElementById(\'hdn_type\'),\''.$documentId.'\',document.getElementById(\'hdn_doc_id\'))" value ="'.$arrresult[$index]['PK_DOC'].'" /></td>';
					$html .='<td onclick="set_hidden_record(this,document.getElementsByName(\'chk_item_id\'),\''.$documentId.'\',\''.$arrresult[$index]['C_DOC_TYPE'].'\',document.getElementById(\'hdn_type\'),document.getElementById(\'hdn_doc_id\'));" align="center"  style="padding-left:3px;padding-right:3px;" class="normal_label" >'.($iStt + 1).'</td>';													
					$html .='<td onclick="set_hidden_record(this,document.getElementsByName(\'chk_item_id\'),\''.$documentId.'\',\''.$arrresult[$index]['C_DOC_TYPE'].'\',document.getElementById(\'hdn_type\'),document.getElementById(\'hdn_doc_id\'));" align="center"  style="padding-left:3px;padding-right:3px;" class="normal_label">'.$sReleaseDate.'&nbsp;</td>';
					$html .='<td onclick="set_hidden_record(this,document.getElementsByName(\'chk_item_id\'),\''.$documentId.'\',\''.$arrresult[$index]['C_DOC_TYPE'].'\',document.getElementById(\'hdn_type\'),document.getElementById(\'hdn_doc_id\'));" align="center" valign="middle"  style="padding-left:3px;padding-right:3px;" class="normal_label">'.$sSymbol.'</td>';
					$html .='<td onclick="set_hidden_record(this,document.getElementsByName(\'chk_item_id\'),\''.$documentId.'\',\''.$arrresult[$index]['C_DOC_TYPE'].'\',document.getElementById(\'hdn_type\'),document.getElementById(\'hdn_doc_id\'));" align="left" style="padding-left:3px;padding-right:3px;" class="normal_label">'.$sSubject.'</td>';
					$html .='<td onclick="set_hidden_record(this,document.getElementsByName(\'chk_item_id\'),\''.$documentId.'\',\''.$arrresult[$index]['C_DOC_TYPE'].'\',document.getElementById(\'hdn_type\'),document.getElementById(\'hdn_doc_id\'));" align="left" style="padding-left:3px;padding-right:3px;" class="normal_label">'.$sReleasePlace.'</td>';
					$html .='</tr>';	
				}		
