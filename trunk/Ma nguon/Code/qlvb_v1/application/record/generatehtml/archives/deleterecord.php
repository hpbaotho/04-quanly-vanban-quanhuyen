...
// Dinh nghia duong dan den thu vien cua Zend
	...
	$myobject	 =  new Zend_Controller_Action_Helper_Redirector();
	...
	$Result					 = $objRecordArchive->DocRecordArchivesDelete($sRecordArchiveIdList,0);
	if($Result[0]['RET_ERROR'] != null || $Result[0]['RET_ERROR'] != '')
		echo "<script type='text/javascript'>var t = alert('".$Result[0]['RET_ERROR']."')</script>";
	else{
		echo "<input type='hidden' id='doc_id_list' value = ".$Result[0]['C_DOC_ID_LIST']."/>";
	}
	$myobject->_redirect('record/archives/index/');	
?>