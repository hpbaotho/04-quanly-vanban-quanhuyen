...
{
$where = db_create_in($_COOKIE['ECS']['history'], 'goods_id');
$sql = 'SELECT goods_id, goods_name,goods_img,shop_price,goods_thumb FROM ' . $GLOBALS['ecs']->table('goods') .
" WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
$query = $GLOBALS['db']->query($sql);
...
$arr[$key]['name'] = $res[$val]['goods_name'];
$arr[$key]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? sub_str($goods_name, $GLOBALS['_CFG']['goods_name_length']) : $goods_name;
$arr[$key]['url'] = build_uri('goods', array('gid' => $val), $goods_name);
$arr[$key]['thumb'] = get_image_path($res[$val]['goods_id'], $res[$val]['goods_thumb'], true);
$arr[$key]['shop_price'] = $res[$val]['shop_price'];
$str.='<LI class=fore'.$key.' ><DIV class="p-img ld"><A href="'.$arr[$key]['url'].'" target=_blank><IMG height=130 alt="'.$arr[$key]['name'].'"  src="'.$arr[$key]['thumb'].'" width=130 ></A></DIV><DIV class=p-name><A title="'.$arr[$key]['name'].'" href="'.$arr[$key]['url'].'" target=_blank>'.$arr[$key]['name'].'</A></DIV><DIV class=p-price>'.$GLOBALS['_LANG']['shop_price'].'<STRONG>'.$arr[$key]['shop_price'].'</STRONG></DIV></LI>';
}
}
}
