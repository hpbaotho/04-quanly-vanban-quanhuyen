...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    /* 初始化“页面标题”和“当前位置” */
    $page_title = 'Nội Thất Online';
    $ur_here    = '<a href="http://www.noithatonline.vn/" rel="nofollow">' . $GLOBALS['_LANG']['home'] . '</a>';
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= ' &gt;<span typeof="v:Breadcrumb"> <a rel="v:url" property="v:title" href="' . build_uri($type, $args, $val['cat_name']) . '">' .
...
function upload_file($upload, $type)
{
    if (!empty($upload['tmp_name']))
    {
        ...
        if (!empty($ftype))
        {
            ...
            $name = $_SESSION['user_id'] . '-' . $name . '.' . $ftype;
