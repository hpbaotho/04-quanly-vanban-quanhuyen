...
//LSJ增加
function get_categories_tree_re($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
...
function get_promote_goods($cats = '')
{
    ...
    foreach ($result AS $idx => $row)
    {
		...
		if($row['promote_price'] >0)
		{
		$goods[$idx]['zhekou']= number_format($row['promote_price'] / $row['market_price'] * 10,1);
		}
		else
		{
		$goods[$idx]['zhekou']="10";
...
function get_goods_fittings($goods_list = array())
{
    ...
    {
        //LSJ
		$arr[$temp_index]['goods_brief']         = $row['goods_brief'];
		//
