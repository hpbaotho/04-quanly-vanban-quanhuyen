...
abstract class Sys_Publib_Library {	
	...
	public function _generateSelectOption($arr_list,$IdColumn,$ValueColumn,$NameColumn,$SelectedValue){
		...
		for($row_index = 0;$row_index< $count;$row_index++){
			...
			$gt=$SelectedValue;
			if(($strID != $SelectedValue) & ('{'.$strID.'}'!=$SelectedValue)) {
			 	$optSelected="";
			} else {
	...
	public function _generateChangeRecordNumberPage($piValue, $pAction){
		...
		$arrListItem = Sys_Xml::_convertXmlStringToArray($psXmlDataInUrl, "item");		
		//Tao chuoi HTML
...
//********************************************************************************************************************
function _builtXMLTreeByOrder($arr_all_list,$exception_brand_id,$show_control,$opening_node_img_name,$closing_node_img_name,$leaf_node_img_name,$select_parent) {
	global $_MODAL_DIALOG_MODE;
	$objConfig = new Sys_Init_Config();
	$sPath = $objConfig->_setImageUrlPath();
	$sUrl = $objConfig->_setWebSitePath();
	$strTop='<?xml version="1.0" encoding="UTF-8"?>'."\n";
	$strTop.='<treeview title="Treeview" >'."\n";
	$strTop.="<custom-parameters>"."\n";
	$strTop.='<param name="shift-width" value="10"/>' . "\n";
	...
	$strTop.='<param name="closing_node_img_name" value="'.$sPath.$closing_node_img_name.'"/>'."\n";	
	$strTop.='<param name="leaf_node_img_name" value="'.$sPath . $leaf_node_img_name.'"/>' . "\n";
	//path chua url de submit
	$strTop.='<param name="url" value="'.$sUrl . 'user/org/index/'.'"/>' . "\n";
	$strTop.='<param name="v_url" value="'.$sUrl . 'user/org/edit/'.'"/>' . "\n";
	$strTop.='<param name="v_url_edit_staff" value="'.$sUrl . 'user/org/editstaff/'.'"/>' . "\n";
	$strTop.='<param name="modal_dialog_mode" value="1"/>' . "\n";
	$strTop.='<param name="show_control" value="'.$show_control.'"/>'."\n";
	$strTop.='<param name="select_parent" value="'.$select_parent.'"/>'."\n";
	$strTop.="</custom-parameters>"."\n";	
	$strBottom= "</treeview>";
	...
	for($i=0;$i<$v_count;$i++){
		$v_current_id = $arr_all_list[$i]['PK_OBJECT'];//PK
		$v_parent_id = $arr_all_list[$i]['FK_OBJECT'];//FK	
		$v_current_code = htmlspecialchars($arr_all_list[$i]['PK_OBJECT']);//C_CODE
		$v_current_name = htmlspecialchars($arr_all_list[$i]['C_NAME']);//C_NAME
		$v_current_type = $arr_all_list[$i]['C_TYPE'];//C_TYPE
		$v_current_level = $arr_all_list[$i]['C_LEVEL'];//C_LEVEL
		$v_current_have_children = $arr_all_list[$i]['C_HAVE_CHILDREN'];//C_HAVE_CHILDREN
		$v_current_internal_order = $arr_all_list[$i]['C_INTERNAL_ORDER'];//C_INTERNAL_ORDER
		if ($i+1<$v_count){
			$v_next_internal_order = $arr_all_list[$i+1]['C_INTERNAL_ORDER'];//C_INTERNAL_ORDER
		}	
		if ($v_current_id==$exception_brand_id){
...
function _write_file($p_file_path, $p_content){
     ...
     *Idea: Gui mail
     *parameter:
     * 		@param  $from: Dia chi hom thu gui vidu:tncdt2k48@gmail.com
     *	 	@param  $pass: pass hom thu
     * 		@param  $to_name :Ho va ten nguoi nhan
     *  	@param  $to :Dia chi mail nguoi nhan
     *		@param  $from_name :Ho va ten nguoi gui
     */
