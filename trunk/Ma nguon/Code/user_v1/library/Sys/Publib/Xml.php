...
class Sys_Publib_Xml extends RAX {		
	...
	public function _xmlGetXmlTagValue($spXmlString, $spXmlParentTag, $spXmlTag){		
		...
		if ($spXmlString != ""){
			...
			return Sys_Library::_restoreXmlBadChar($objXmlData->$spXmlTag);
		}else{
			return "";
		}
	}	
	...
	*/
	...
	public function _xmlGenerateFormfield($spXmlFileName, $pathXmlTagStruct,$pathXmlTag, $p_xml_string_in_db, $p_arr_item_value,$p_input_file_name=true,$p_view_mode=false){		
		...
		foreach ($arrTable_truct_rows as $row){
			...
			for($i=0;$i < sizeof($arr_tag);$i++){
				...
				if($this->viewPosition == 'left'){
					$sContentXmlTopLeft .= Sys_Publib_Xml::_generateHtmlInput();	
				}
				else if($this->viewPosition == 'right'){
					$sContentXmlTopRight .= Sys_Publib_Xml::_generateHtmlInput();	
				}
				else {
					$sContentXmlBottom .= Sys_Publib_Xml::_generateHtmlInput();	
				}	
			}
			if($this->viewPosition == 'left')
		...
		if($v_js_file_name != '' && !(is_null($v_js_file_name))){
			$spHtmlStr .= Sys_Publib_Library::_getAllFileJavaScriptCss('','js/js-record',$v_js_file_name,',','js');
		...
		switch($this->spType){
			...
			case "relaterecord";
				...
				if ($v_value == ""){
					...
					if ($v_record_id>0){
						$v_ret_html = $v_ret_html . "<a href='".$_Sys_WEB_SITE_PATH."record/archives/index.php?fuseaction=DISPLAY_SINGLE_LICENSE&hdn_recordtype_filter=$v_recordtype&hdn_record_id=$v_record_id'>N&#7897;i dung c&#7911;a h&#7891; s&#417; li&#234;n quan</a>";
					}
				}
				break;	
			case "file_upload";
			...
			case "fileclient";
				...
				$spRetHtml = $v_str_label;				
				$spRetHtml = $spRetHtml ."<div name='$this->formFielName' style='display:none'><input id = '$this->formFielName' type='text' name='$this->formFielName'class='normal_textbox' title='$this->tooltip' value='$this->value'  style='width:$this->width' style='border=0' readonly  $this->sDataFormatStr xml_tag_in_db='$this->xmlTagInDb' xml_data='$this->xmlData' column_name='$this->columnName' message='$this->spMessage' ". Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional).Sys_Publib_Xml::_generatePropertyType("readonly",$this->readonlyInEditMode).Sys_Publib_Xml::_generatePropertyType("disabled",$this->disabledInEditMode)."></div>";
			...
			case "selectbox";
				...
				if ($this->viewMode && $this->readonlyInEditMode=="true"){
					if ($this->inputData == "session"){
					...
					}elseif ($this->inputData == "Syslist"){//Lay du lieu tu file XML					
						...
						$arrListItem = Sys_DB_Connection::adodbQueryDataInNumberMode($this->selectBoxOptionSql,$this->cacheOption);											
						if ( $this->theFirstOfIdValue =="true" && $this->value == "" ){
							$this->value = $arrListItem[0][$this->selectBoxIdColumn];
						}
						$spRetHtml = $spRetHtml .Sys_Publib_Xml::_getValueFromArray($arrListItem,$this->selectBoxIdColumn,$this->selectBoxNameColumn,$this->value);
			...
			case "multiplecheckbox";
				$spRetHtml = $v_str_label;
				if ($this->inputData == "session"){
					$spRetHtml = $spRetHtml . "<div style='display:none'><input type='textbox' id='$this->formFielName' name='$this->formFielName' value='' hide='true' readonly " . Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional) . "xml_data='true' xml_tag_in_db='$this->xmlTagInDb' message='$this->spMessage'></div>";
					$spRetHtml = $spRetHtml . Sys_Publib_Xml::_generateHtmlForMultipleCheckboxFromSession($this->sessionName, $this->checkBoxMultipleIdColumn,$this->checkBoxMultipleNameColumn,$this->value);
				}elseif ($this->inputData == "Syslist"){
					$v_xml_data_in_url = Sys_Publib_Library::_readFile($this->SysListWebSitePath."xml/list/output/".$this->publicListCode.".xml");
					$spRetHtml = $spRetHtml . Sys_Publib_Xml::_generateHtmlForMultipleCheckbox(Sys_Publib_Xml::_convertXmlStringToArray($v_xml_data_in_url,"item"),$this->checkBoxMultipleIdColumn,$this->checkBoxMultipleNameColumn,$this->value);
				}else{		
					//thay the ma don vi cua nguoi dang nhap hien thoi vao chuoi SQL												
					$this->checkBoxMultipleSql = str_replace("#OWNER_CODE#",$_SESSION['OWNER_CODE'],$this->checkBoxMultipleSql);	
					$spRetHtml = $spRetHtml . "<div style='display:none'><input type='textbox' id='$this->formFielName' name='$this->formFielName' value='' hide='true' readonly " . Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional) . "xml_data='true' xml_tag_in_db='$this->xmlTagInDb' message='$this->spMessage'></div>";
					$spRetHtml = $spRetHtml . Sys_Publib_Xml::_generateHtmlForMultipleCheckbox(Sys_DB_Connection::adodbQueryDataInNumberMode($this->checkBoxMultipleSql,$this->cacheOption),$this->checkBoxMultipleIdColumn,$this->checkBoxMultipleNameColumn,$this->value);
				}
				break;
			...
			case "multiplecheckbox_fileAttach";
				...
				if ($this->inputData == "session"){
					...
					$v_xml_data_in_url = Sys_Publib_Library::_readFile($this->SysListWebSitePath."xml/list/output/".$this->publicListCode.".xml");
					$spRetHtml = $spRetHtml . Sys_Publib_Xml::_generateHtmlForMultipleCheckbox_fileAttach(Sys_Publib_Xml::_convertXmlStringToArray($v_xml_data_in_url,"item"),$this->checkBoxMultipleIdColumn,$this->checkBoxMultipleNameColumn,$this->value);
				}else{		
					//thay the ma don vi cua nguoi dang nhap hien thoi vao chuoi SQL												
					$this->checkBoxMultipleSql = str_replace("#OWNER_CODE#",$_SESSION['OWNER_CODE'],$this->checkBoxMultipleSql);	
					$spRetHtml = $spRetHtml . "<div style='display:none'><input type='textbox' id='$this->formFielName' name='$this->formFielName' value='' hide='true' readonly " . Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional) . "xml_data='true' xml_tag_in_db='$this->xmlTagInDb' message='$this->spMessage'></div>";
					$spRetHtml = $spRetHtml . Sys_Publib_Xml::_generateHtmlForMultipleCheckbox_fileAttach(Sys_DB_Connection::adodbQueryDataInNumberMode($this->checkBoxMultipleSql,$this->cacheOption),$this->checkBoxMultipleIdColumn,$this->checkBoxMultipleNameColumn,$this->value);
				}
				break;
			case "multipleradio";
				if ($this->inputData == "Syslist"){				
					...
					$arrListItem = Sys_DB_Connection::adodbQueryDataInNumberMode($this->checkBoxMultipleSql,$this->cacheOption);
				}
			
				if($this->direct == 'true'){
					$spRetHtml = $this->sLabel.$this->optOptionalLabel;
					$spRetHtml = $spRetHtml . "<input type='text' name='$this->formFielName' value='$this->value' hide='true' readonly ".Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional)." xml_data='true' xml_tag_in_db='$this->xmlTagInDb' message='$this->spMessage'>";
					...
					if($this->sLabel != "" && isset($this->sLabel)){
						$spRetHtml = $this->sLabel.$this->optOptionalLabel;
						if ($this->inputData == "session"){
							$spRetHtml = $spRetHtml . "<input type='text' name='$this->formFielName' value='' hide='true' readonly ".Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional)." xml_data='true' xml_tag_in_db='$this->xmlTagInDb' message='$this->spMessage'>";
			...
			case "multipletextbox";
				if ($this->inputData == "Syslist"){
					...
					$arrListItem = Sys_DB_Connection::adodbQueryDataInNumberMode($this->textBoxMultipleSql,$this->cacheOption);
				}				
				$spRetHtml = $this->sLabel;
				$spRetHtml = $spRetHtml . "<input type='text' id='$this->formFielName' name='$this->formFielName' value='' hide='true' readonly ".Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional)." xml_data='true' xml_tag_in_db='$this->xmlTagInDb' message='$this->spMessage'>";
			...
			case "treeuser";		
				$spRetHtml = $this->sLabel;
				$spRetHtml = $spRetHtml . "<input type='text' style='display:none' id='$this->formFielName' name='$this->formFielName' value='' hide='true' readonly ".Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional)." xml_data='true' xml_tag_in_db='$this->xmlTagInDb' message='$this->spMessage'>";
			...
			case "textboxorder";
				...
				if(is_null($this->value) || $this->value==""){
					$this->value = Sys_Library::_getNextValue("T_SysLIB_LIST","C_ORDER","FK_LISTTYPE = " . $_SESSION['listtypeId']);					
			...
			case "button";
				...
				$spRetHtml = $spRetHtml . "&nbsp;&nbsp;<input type='button' name='$this->formFielName' class='$this->className' value='$this->sLabel' title='$this->tooltip' style='width:$this->width' ".Sys_Publib_Xml::_generatePropertyType("optional",$this->optOptional).Sys_Publib_Xml::_generatePropertyType("readonly",$this->readonlyInEditMode).Sys_Publib_Xml::_generatePropertyType("disabled",$this->disabledInEditMode).Sys_Publib_Xml::_generateEventAndFunction($this->jsFunctionList, $this->jsActionList)." $this->sDataFormatStr xml_tag_in_db='$this->xmlTagInDb' xml_data='$this->xmlData' column_name='$this->columnName' message='$this->spMessage' onClick='$this->onclickFunction'>";
				$spRetHtml = $spRetHtml . $this->note;
				break;
			case "hidden";	
			...
			case "channel";
				if ($this->inputData != "session"){
					...
					$arr_all_channel = Sys_DB_Connection::adodbQueryDataInNumberMode($this->channelSql,$this->cacheOption);
				}else{
	...
	public function _generateHtmlForMultipleCheckboxFromSession($p_session_name, $p_session_id_index,$session_name_index,$p_valuelist,$p_height ='auto') { 
		...
		if($this->dspDiv == 1){// = 1 thi hien di DIV
			$strHTML = $strHTML ."<DIV title='$this->tooltip' STYLE='overflow: auto; height:$p_height;padding-left:0px;margin:0px;'>";
			$strHTML = $strHTML . "<table id = 'table_$this->formFielName' class='list_table2'  width='99%' cellpadding='0' cellspacing='0'><col width='2%'><col width='48%'><col width='2%'><col width='48%'>";
		}else{		
			$strHTML = $strHTML . "<table id = 'table_$this->formFielName' class='list_table2'  width='99%' cellpadding='0' cellspacing='0'><col width='2%'><col width='98%'>";
		}	
		$i = 0;				
	...
	public function _xmlGenerateList($psXmlFile, $psXmlTag, $pArrAllItem, $psColumeNameOfXmlString, $NamOfColId, $sFullTextSearch, $pHaveMove=false, $pOnclick= false,$sAction = ''){	
		...
		if(!$pOnclick){
			$psHtmlString = $psHtmlString  . Sys_Library::_addEmptyRow(sizeof($pArrAllItem),15,$v_current_style_name,sizeof($arrTable_Struct));
		}else{
			$psHtmlString = $psHtmlString  . Sys_Library::_addEmptyRow(sizeof($pArrAllItem),15,$v_current_style_name,sizeof($arrTable_Struct));
		...
		switch($pType) {
			case "checkbox";
				...
				if ($v_id_column =="true" && $v_have_move){
					if ($row_index !=0){
						$psRetHtml = $psRetHtml. '<img src="'.$this->SysImageUrlPath.'/up.gif" border="0" style="cursor:pointer;" ondbClick="'.$v_onclick_up.'">';
					}else{
						$psRetHtml = $psRetHtml. '&nbsp;&nbsp;&nbsp;';
					}
					if ($row_index != $this->count-1){
						$psRetHtml = $psRetHtml. '<img src="'.$this->SysImageUrlPath.'/down.gif" border="0" style="cursor:pointer;" ondbClick="'.$v_onclick_down.'">';
					}else{
						$psRetHtml = $psRetHtml. '&nbsp;&nbsp;&nbsp;&nbsp;';
					}
				}
			...
			case "selectbox";			
				if ($this->inputData == "Syslist"){
					...
					//thay the ma don vi cua nguoi dang nhap hien thoi vao chuoi SQL												
					$this->selectBoxOptionSql = str_replace("#OWNER_CODE#",$_SESSION['OWNER_CODE'],$this->selectBoxOptionSql);
					$arr_list_item = Sys_DB_Connection::adodbQueryDataInNumberMode($this->selectBoxOptionSql,$this->cacheOption);					
				} 
			...
			case "date";
				$sDate = Sys_Function_RecordFunctions::searchCharColor($this->sFullTextSearch,$objSysLib->_yyyymmddToDDmmyyyy($this->value));
			...
			case "time";
				$psRetHtml = '<td class="data" align="'.$this->v_align.'" onclick="set_hidden(this,document.getElementsByName(\'chk_item_id\'),document.getElementById(\'hdn_list_id\'),\''.$objectId.'\')" ondblclick="'.$sAction.'">'.'&nbsp;'. Sys_Function_RecordFunctions::searchCharColor($this->sFullTextSearch,$objSysLib->_yyyymmddToHHmm($this->value)).'&nbsp;</td>';
				break;
				
			case "text";
				if($this->xmlTagInDb=='ho_ten_nk'){
					
					$psRetHtml = '<td class="data" align="'.$this->v_align.'" onclick="set_hidden(this,document.getElementsByName(\'chk_item_id\'),document.getElementById(\'hdn_list_id\'),\''.$objectId.'\')" ondblclick="'.$sAction.'">'.Sys_Function_RecordFunctions::searchStringColor2($this->sFullTextSearch, $this->value).'&nbsp;</td>';
					
				}
				else 
					$psRetHtml = '<td class="data" align="'.$this->v_align.'" onclick="set_hidden(this,document.getElementsByName(\'chk_item_id\'),document.getElementById(\'hdn_list_id\'),\''.$objectId.'\')" ondblclick="'.$sAction.'">'.Sys_Function_RecordFunctions::searchStringColor($this->sFullTextSearch, $this->value).'&nbsp;</td>';
				break;
			case "char";
				$psRetHtml = '<td class="data" align="'.$this->v_align.'" onclick="set_hidden(this,document.getElementsByName(\'chk_item_id\'),document.getElementById(\'hdn_list_id\'),\''.$objectId.'\')" ondblclick="'.$sAction.'">'.'&nbsp;'.Sys_Function_RecordFunctions::searchCharColor($this->sFullTextSearch, $this->value).'&nbsp;</td>';
				break;
			case "identity";
			...
			case "money";
				$psRetHtml = '<td class="data" align="'.$this->v_align.'" onclick="set_hidden(this,document.getElementsByName(\'chk_item_id\'),document.getElementById(\'hdn_list_id\'),\''.$objectId.'\')" ondblclick="'.$sAction.'">'.Sys_Function_RecordFunctions::searchCharColor($this->sFullTextSearch,_dataFormat($this->value)).'&nbsp;</td>';
				break;						
			default:
		...
		foreach ($arrfilter_rows as $rows){
			...
			for($i=0;$i < sizeof($arr_tag); $i++){
				...
				if ($p_filter_xml_string!=""){
					...
					$value_input = Sys_Library::_replaceXmlBadChar($value);
...
public function _generateHtmlForMultipleCheckbox($arrList, $IdColumn, $NameColumn, $Valuelist, $arrPara = array(),$height='auto') {
	...
	public function _generateHtmlForMultipleCheckbox_fileAttach($arrList, $IdColumn, $NameColumn, $Valuelist, $arrPara = array(),$height='auto') {
		...
		if ($count_item > 0){
			...
			$urlFileAttach=new Sys_Init_Config();
			//ma ho so
			$v_record_id = $_SESSION['NET_RECORDID'];
			//var_dump($arrList);exit;
			$modSendRecord=new Sys_Function_RecordFunctions();
			while ($i<$count_item) {
				$v_item_id = $arrList[$i][$IdColumn];
				$v_item_name = $arrList[$i][$NameColumn];
				...
				if($this->dspDiv == 1){
					...
					$strHTML = $strHTML . "<td style='width:38%' style='display:'><input type=file optional=true size='48%' class=small_textbox name=dk_file_attach$i><a href='$v_delete_file $v_delete_file_upload' > Delete  </a> </td></tr>";
				}	
				if($i % 2 != 1 && $i == $count_item - 1 && $this->dspDiv == 1){
					$strHTML = $strHTML . "<td colspan = \"2\"> </td>";
				}
	...
	switch($v_type) {
		...
		case "selectbox";
			...
			if ($this->inputData == "session"){
				...
				$v_xml_data_in_url = Sys_Publib_Library::_readFile($_Sys_LIST_WEB_SITE_PATH."listxml/output/".$this->publicListCode.".xml");
				...
				$v_ret_html = $v_ret_html .self::_getValueFromArray(Sys_DB_Connection::adodbQueryDataInNumberMode($this->selectBoxOptionSql,$this->cacheOption),$this->selectBoxIdColumn,$this->selectBoxNameColumn,$this->value);
			}
			break;
		default:
			$v_ret_html = "";
