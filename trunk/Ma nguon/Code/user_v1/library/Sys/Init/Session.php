...
class Sys_Init_Session extends Zend_Session_Namespace {	
	...
	public function SesGetPersonalInfoOfAllStaff($p_webservice_path = ""){
		...
		//$p_delimitor = ",";		
		$arrResul = array();
		$sql = "Exec DBLink.[Sys-user-v4].dbo.USER_GetPersonalInfoOfAllStaff ";
		try{
...
//		}		
	...
	public function SesGetDetailInfoOfAllUnit($p_webservice_path = ""){
		...
		//$p_delimitor = ",";
		$arrResul = array();
		$sql = "Exec DBLink.[Sys-user-v4].dbo.USER_GetDetailInfoOfAllUnit ";
		try{
			$arrResul = Sys_DB_Connection::adodbQueryDataInNameMode($sql);
		}catch (Exception $e){
			echo $e->getMessage();
...
//		}	
	...
	public function SesGetAllOwner($option=''){
		$arrOwner = array();
		$v_root_id = Sys_Library::_getRootUnitId($_SESSION['arr_all_unit_keep']);
		if($option == ''){
			...
			foreach($_SESSION['arr_all_unit_keep'] as $objUnit){
				if($objUnit['parent_id']== $v_root_id && $objUnit['code'] !=  $_SESSION['OWNER_CODE']){
					$arr1Owner = array("id"=>$objUnit['id'],"name"=>$objUnit['name'],"code"=>$objUnit['code'],"address"=>$objUnit['address'],"email"=>$objUnit['email'],"order"=>$objUnit['order']);
					array_push($arrOwner,$arr1Owner);
				}
			}
