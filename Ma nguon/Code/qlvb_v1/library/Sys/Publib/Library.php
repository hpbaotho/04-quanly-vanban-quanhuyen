...
/**
 * Noi dung: Tao lop Sys_Publib_Library luu cac ham dung chung
 */
abstract class Sys_Publib_Library {	
	/**
	 ...
	 * @return Xau HTML
	 */
	public function _generateChangeRecordNumberPage($piValue, $pAction, $pRowtype = ''){
		//Load class
		...
		$psXmlDataInUrl = Sys_Library::_readFile("./xml/list/output/SO_HS_TREN_TRANG.xml");		
		//Chuyen doi thong tin trong xau XML -> Mang
		$arrListItem = Sys_Publib_Xml::_convertXmlStringToArray($psXmlDataInUrl, "item");		
		//Tao chuoi HTML
		...
		$psHtmlString = $psHtmlString . Sys_Library::_generateSelectOption($arrListItem, 'c_code', 'c_code', 'c_name', $piValue);
		$psHtmlString = $psHtmlString . "</select>&nbsp;";	
		$psHtmlString = $psHtmlString . $pRowtype . "/1 trang";		
		return $psHtmlString;
	}
	...
	public function _getAllFileJavaScriptCss($psModuleName,$psModuleNameOrther = "" ,$parrFileName = "",$psDelimitor = ",",$psExtension){		
		...
		$objInitConfif  = new Sys_Init_Config();
	...
	public function _getAllFileAttach($strFileList, $deliFileList = '!#~$|*', $deliFileName = '!~!', $sPathLink){
		...
		for($i =0; $i < sizeof($strFileName); $i ++){
			...
			//echo $baseUrl;
			$linkImg = " <img src = '" . $baseUrl . "public/images/file_attach.gif'/>";
	...
	/*
	 * Lay duong dan File xuat ban XML
	 * */
	public function _getAllFileAttachForXMLRSS($strFileList, $deliFileList = '!#~$|*', $deliFileName = '!~!', $sPathLink){
		$strFileName = explode($deliFileList,$strFileList);
		for($i =0; $i < sizeof($strFileName); $i ++){
			$arrFileName = explode($deliFileName,$strFileName[$i]);
			$fileName    = $arrFileName[1];
			...
			$fileLink = $sPathLink . $arrFileLink[0] . "/" . $arrFileLink[1] . "/" . $arrFileLink[2] . "/" . $strFileName[$i];
			$pathFileExist = str_replace("/","\\",$fileLink);
			$fileAttach = self::_replaceBadChar($fileLink);				
		}
		return $fileAttach; 
	}
	/**	 
	 ...
	 * @param $deliFileName: ky tu phan tach giua ID va Name cua mot file dinh kem
	 * @param $sPathLink: duong dan toi folder chua file dinh kem can hien thi
	 * @param $sfuntion_onclick: ham onclick keo theo khi chon mot ban ghi
	 * @return tra ve danh sach cac file dinh kem
	 */
	public function _getAllFileAttachAndSubmit($strFileList, $deliFileList = '!#~$|*', $deliFileName = '!~!', $sPathLink, $sfuntion_onclick){
		//echo 'ok'.$strFileList;
		$strFileName = explode($deliFileList,$strFileList);
		for($i =0; $i < sizeof($strFileName); $i ++){
			$arrFileName = explode($deliFileName,$strFileName[$i]);
		...
		/*	//neu la file tieng viet co dau
			...
			//neu la file tieng viet khong dau
			if(file_exists($pathFileExist)){				
				$fileAttach = $fileAttach . $linkImg . "<a target='blank' onclick='".$sfuntion_onclick."' href = '" . self::_replaceBadChar($fileLink) ."' title = 'file dinh kem'>" . $fileName . "</a>";
			}else{
				$fileLink = $sPathLink . $arrFileLink[0] . "/" . $strFileName[$i];
				$pathFileExist = '..' . str_replace("/","\\",$fileLink);
				if(file_exists($pathFileExist)){
...
/**
	...
	//Lay danh sach tat ca cac phogn ban cua tat ca cac don vi trien khai
	public function _getArrAllOwner(){
		try {
			$i = 0;
			foreach($_SESSION['arr_all_unit_keep'] as $v_unit){
				$arr_child_unit[$i][0] = $v_unit['id'];
				$arr_child_unit[$i][1] = $v_unit['parent_id'];
				$arr_child_unit[$i][2] = $v_unit['code'];
				$arr_child_unit[$i][3] = $v_unit['name'];
	...
	public function _getArrChildStaff($arr_unit){
		try {
			...
			for($j=0; $j < $v_count; $j++){
				foreach($_SESSION['arr_all_staff_keep'] as $v_staff){
	...
	public function _InforStaff(){
		...
		$piUnitName = Sys_Publib_Library::_getItemAttrById($_SESSION['arr_all_unit'],$piUnitId,'name');			
		return $StaffName."-".$piUnitName;
	}	
	 ...
	 */
	public function _getInformationStaffLogin($sStaffName = '', $sPositionName = '', $sUnitName = '', $sUserName){		
		$strHtml = "";
		if ($sStaffName != ""){
			...
			$http = 'http://'.$_SERVER['HTTP_HOST'].Sys_Init_Config::_setWebSitePath().'login/index/changepass/';
			$strHtml .= '<tr><td class="level_dangnhap"><a  onclick="getFormUpdatePassword(\''.$http.'\',\''. $sUserName .'\');"> Đổi mật khẩu </a></td></tr>';
			$strHtml .= "</table>";
		}			
		return $strHtml;						
	}
	public function _UserInfo($sStaffName = '', $sPositionName = '', $sUnitName = '', $sUserName){		
		$strHtml = "";		
		$strHtml .= 'Xin chào: '.$sStaffName.' - Đơn vị: '.$sUnitName;
		return $strHtml;						
	}
    ...
    /**
     * cuongnh//chuyen chu co dau sang telex va nguoc lai
     */
    function _convertUnicodeToETX($strText){
    	$uniChars = array("á","à","ả","ã","ạ","ă","ắ","ằ","ẳ","ẵ","ặ","â","ấ","ầ","ẩ","ẫ","ậ","é","è","ẻ","ẽ","ẹ","ê","ế","ề","ể","ễ","ệ","í","ì","ỉ","ĩ","ị","ó","ò","ỏ","õ","ọ","ô","ố","ồ","ổ","ỗ","ộ","ơ","ớ","ờ","ở","ỡ","ợ","ú","ù","ủ","ũ","ụ","ư","ứ","ừ","ử","ữ","ự","ý","ỳ","ỷ","ỹ","ỵ","đ","Á","﻿À","Ả","Ã","Ạ","Ă","Ắ","Ằ","Ẳ","Ẵ","Ặ","Â","Ấ","Ầ","Ẩ","Ẫ","Ậ","É","È","Ẻ","Ẽ","Ẹ","Ê","Ế","Ề","Ể","Ễ","Ệ","Í","Ì","Ỉ","Ĩ","Ị","Ó","Ò","Ỏ","Õ","Ọ","Ô","Ố","Ồ","Ổ","Ỗ","Ộ","Ơ","Ớ","Ờ","Ở","Ỡ","Ợ","Ú","Ù","Ủ","Ũ","Ụ","Ư","Ứ","Ừ","Ử","Ữ","Ự","Ý","Ỳ","Ỷ","Ỹ","Ỵ","Đ");
    	$etxChars = array("as","af","ar","ax","aj","aw","aws","awf","awr","awx","awj","aa","aas","aaf","aar","aax","aaj","es","ef","er","ex","ej","ee","ees","eef","eer","eex","eej","is","if","ir","ix","ij","os","of","or","ox","oj","oo","oos","oof","oor","oox","ooj","ow","ows","owf","owr","owx","owj","us","uf","ur","ux","uj","uw","uws","uwf","uwr","uwx","uwj","ys","yf","yr","yx","yj","dd","AS","AF","AR","AX","AJ","AW","AWS","AWF","AWR","AWX","AWJ","AA","AAS","AAF","AAR","AAX","AAJ","ES","EF","ER","EX","EJ","EE","EES","EEF","EER","EEX","EEJ","IS","IF","IR","IX","IJ","OS","OF","OR","OX","OJ","OO","OOS","OOF","OOR","OOX","OOJ","OW","OWS","OWF","OWR","OWX","OWJ","US","UF","UR","UX","UJ","UW","UWS","UWF","UWR","UWX","UWJ","YS","YF","YR","YX","YJ","DD");
    	for($i =0; $i <sizeof($uniChars); $i ++){
			$strText = str_replace($uniChars[$i], $etxChars[$i], $strText);
		}	
		return 	$strText; 
    }
    function _convertETXToUnicode($strText){
    	$uniChars = array("Ứ","Ừ","Ử","Ữ","Ự","Ớ","Ờ","Ở","Ỡ","Ợ","Ố","Ồ","Ổ","Ỗ","Ộ","Ế","Ề","Ể","Ễ","Ệ","Ấ","Ầ","Ẩ","Ẫ","Ậ","Ắ","Ằ","Ẳ","Ẵ","Ặ","ứ","ừ","ử","ữ","ự","ớ","ờ","ở","ỡ","ợ","ố","ồ","ổ","ỗ","ộ","ế","ề","ể","ễ","ệ","ấ","ầ","ẩ","ẫ","ậ","ắ","ằ","ẳ","ẵ","ặ","á","à","ả","ã","ạ","ă","â","é","è","ẻ","ẽ","ẹ","ê","í","ì","ỉ","ĩ","ị","ó","ò","ỏ","õ","ọ","ô","ơ","ú","ù","ủ","ũ","ụ","ư","ý","ỳ","ỷ","ỹ","ỵ","đ","Á","﻿À","Ả","Ã","Ạ","Ă","Â","É","È","Ẻ","Ẽ","Ẹ","Ê","Í","Ì","Ỉ","Ĩ","Ị","Ó","Ò","Ỏ","Õ","Ọ","Ô","Ơ","Ú","Ù","Ủ","Ũ","Ụ","Ư","Ý","Ỳ","Ỷ","Ỹ","Ỵ","Đ");
    	$etxChars = array("UWS","UWF","UWR","UWX","UWJ","OWS","OWF","OWR","OWX","OWJ","OOS","OOF","OOR","OOX","OOJ","EES","EEF","EER","EEX","EEJ","AAS","AAF","AAR","AAX","AAJ","AWS","AWF","AWR","AWX","AWJ","uws","uwf","uwr","uwx","uwj","ows","owf","owr","owx","owj","oos","oof","oor","oox","ooj","ees","eef","eer","eex","eej","aas","aaf","aar","aax","aaj","aws","awf","awr","awx","awj","as","af","ar","ax","aj","aw","aa","es","ef","er","ex","ej","ee","is","if","ir","ix","ij","os","of","or","ox","oj","oo","ow","us","uf","ur","ux","uj","uw","ys","yf","yr","yx","yj","dd","AS","AF","AR","AX","AJ","AW","AA","ES","EF","ER","EX","EJ","EE","IS","IF","IR","IX","IJ","OS","OF","OR","OX","OJ","OO","OW","US","UF","UR","UX","UJ","UW","YS","YF","YR","YX","YJ","DD");
    	for($i =0; $i <sizeof($uniChars); $i ++){
			$strText = str_replace($etxChars[$i], $uniChars[$i], $strText);
		}	
		return 	$strText; 
    }
    /**	 
	...
	public function _generateStringNumberPage($piTotalRecord, $piCurrentPage, $piNumberRecordOnList,$pUrl){
		if($piTotalRecord > $piNumberRecordOnList){
			...
			$psHtmlString = $psHtmlString . "<tr align='center' class='paddingPageGoogle'>";
			if($piCurrentPage >1){
				$piCurrentPagepre =$piCurrentPage - 1;
				$psHtmlString = $psHtmlString . "<td width='2%' onclick = 'break_page(\"$piCurrentPagepre\",\"$pUrl\");'>"."<a><b>Trước</b></a>"."</td>";
			}
			...
			if($piCurrentPage < $psNumberPage){
				...
				$psHtmlString = $psHtmlString . "<td width='2%' onclick = 'break_page(\"$piCurrentPagenex\",\"$pUrl\");'>"."<a><b>Tiếp</b></a>" ."</td>";
			}
...
public  function Generate_weeks_of_year($p_year, $p_number_of_week, $p_current_week){		
		$p_week_name = "Tuần ";
		//Xac dinh ngay 1/1 la thu may
		$v_timestamp = mktime(0,0,0,1,1,$p_year);		
		$v_fisrt_day_of_year = date('w',$v_timestamp);
		...
		for ($i=1; $i <= 53; $i++){
			...
			if ($add_HTML == true){
				...
				//$strHTML.='<select name="cbo_week" id="cbo_week" style="width:300px;" class="textbox">';
				$strHTML.='<option id='.'"'.$i.'"'.' name='.'"'.$i.'"';
				$strHTML.=' value='.'"'.$i.'"';
				$strHTML.=' fisrt_day='.'"'.$v_fisrt_day_of_week.'"';
				$strHTML.=' last_day='.'"'.$v_last_day_of_week.'"';
...
public function _generate_year_input($p_begin_year,$p_end_year,$p_current_year){
	$strHTML = "";
...
public  function _get_day_of_week_in_vn($pDate){	
	$RetStr = "";
	switch($pDate){
		case 1;
			$RetStr = "Th&#7913; hai";
...
public function _generate_days_on_week_of_year($p_year, $p_week, $p_full_day,$p_current_day){
	...
	for ($i=1; $i <= 53; $i++){
		if($i == $p_week){
			for ($j=0; $j<7; $j++){
				$v_day_on_week = $this->_get_day_of_week_in_vn(date("w",mktime(0,0,0,1,$v_date+$j,$p_year)));
				...
				if ($p_full_day){
					$v_text = $v_day_on_week.'<br/>'.$v_date_on_week.'!@#';
				}else{
...
function _generate_days($p_year, $p_week, $p_full_day,$p_current_day){	
	$v_timestamp = mktime(0,0,0,1,1,$p_year);
	$v_fisrt_date_of_year = date('w',$v_timestamp);
	if ($v_fisrt_date_of_year == 0) {$v_fisrt_date_of_year = 7;}	
	if ($v_fisrt_date_of_year <= 4){
 ...
 * 
 * Enter description here ...
 * @param unknown_type $v_value
 */
function _isrplatecontent($v_value){
	$v_len = strlen($v_value);
		if($v_len > 0){
			for($index = 0;$index < $v_len;$index++){
				if(ord(substr($v_value,$index,1)) == 10){//=10 la ma xuong dau dong
				$v_value = str_replace(chr(10),"!~~!",$v_value);				
				}
			}
		}
	return $v_value;
