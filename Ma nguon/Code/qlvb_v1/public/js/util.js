...
/**
 * Xoa anh dinh kem trong tin bai
 * @param p_row_obj
 * @param p_checkbox_obj
 * @param fileUrl
 */
function delete_image_exist(p_row_obj, p_row_obj_new, filename, fileUrl, module){
	if(confirm("Bạn thật sự muốn xóa?")){
	try{
		//alert(p_row_obj[0].style);
		p_row_obj[0].style.display = "none";
		p_row_obj_new[0].style.display = "block";
	}
	catch(e){;}
	try{
		arrUrl = fileUrl.split('/');
		var key = 'fileNameList=' + filename + '!#~$|*&module=' + module;
		if(key != ""){
			postAJAXHTTPText('/' + arrUrl[1] + '/public/ajax/deleteFileUpload.php', key, null, null);
		}
	}catch(e){;}
...
function btn_update_onclick(p_checkbox_obj,p_url){
	//alert(p_checkbox_obj.value + '-' + p_url);
...
///////////////////////////////////////////////////////////////
/*function _select_all_multiple_checkbox(f,rdo_id,option){
	for (i=0;i<f.length;i++){
...
function row_onclick_read_mail(p_item_value){		
	document.getElementById('hdn_object_id').value =  p_item_value;	
	actionUrl('../read');		
}
...
//Ham hien thi Dialog o chinh giua man hinh ap dung trong truong hop trinh duyen Firefox
function DialogCenter(pageURL, title,w,h) {
	var left = (screen.width/2)-(w/2);
	var top = (screen.height/2)-(h/2);
	var targetWin = window.open (pageURL, title, 'toolbar=no, dialogCenter=yes, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
} 
function checkvalue(){
	try{
...
/*
 * Ham su dung de goi Form cap nhat Passworld
 * haidv
 */
function getFormUpdatePassword(p_url,username){		
	p_url = p_url + '?username=' + username + '&showModalDialog=1&' + randomizeNumber();		
	if(navigator.appName == 'Netscape'){
		sRtn = DialogCenter(p_url,"THAY ĐỔI MẬT KHẨU NGƯỜI SỬ DỤNG",450,220);
	}else{
		sRtn = showModalDialog(p_url,"","dialogWidth=450px;dialogHeight=220px;status=no;scroll=no;dialogCenter=yes");
	}	
}
