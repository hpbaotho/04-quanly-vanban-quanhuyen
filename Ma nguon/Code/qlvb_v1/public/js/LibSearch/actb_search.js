function actb(obj,hdnobj,ca,fa,fEnter,single,editable,sWebsitePart){
	/* ---- Public Variables ---- */
	...
	var actb_delimcuongnh = new Array();
	var actb_cdelimword = 0;
	var actb_delimchar = new Array();
	var actb_delimcharcuongnh = new Array();
	var actb_display = false;
	var actb_pos = 0;
	var actb_total = 0;
	var actb_curr = null;
	//cuongnh -> bien hdn
	var actb_cuongnhhdn = null;
	var actb_rangeu = 0;
	var actb_ranged = 0;
	...
	this.actb_keywords = fa;
	this.actb_fillKey = fa;
	//cuongnh -> set value
	this.actb_cuongnhlistvalue = ca;
	this.actb_cuongnhvalue = "";
	var actb_self = this;
	
	actb_curr = obj;
	actb_cuongnhhdn = hdnobj;
	...
	function actb_insertword(a,acuongnh){
		if (actb_self.actb_delimiter.length > 0){
			str = '';
			strcuongnh = '';
			l=0;
			//alert(actb_delimwords);
			for (i=0;i<actb_delimwords.length;i++){
				if (actb_cdelimword == i){
					...
					if (gotbreak){
						str += postspace;
						//strcuongnh += postspace;
					}
				}else{
					str += actb_delimwords[i];
					strcuongnh += actb_delimcuongnh[i];
				}
			...
			var arrstrcuongnh = splitArray(strcuongnh,',');
			//alert(arrstr);
			var strtemp = '';
			var strtempcuongnh = '';
			//alert(actb_cdelimword);
			if(editable == 0){
				for(i = 0; i < arrstr.length; i++){
					if(i != actb_cdelimword){
						var name = arrstr[i].trim();
							...
							if(name == actb_self.actb_fillKey[j]){
								if(single == 0){
									if(strtemp.search(arrstr[i].trim()) < 0){
										...
										strtempcuongnh += arrstrcuongnh[i].trim() + ',';	
										//alert(arrstrcuongnh[i]);
									}
								}
								else	{
									if(strtemp.search(arrstr[i].trim()) < 0){
			...
			if(single == 0){
				...
				actb_cuongnhhdn.value = strtempcuongnh;
			}else{
				actb_curr.value = strtemp;
				actb_cuongnhhdn.value = strtempcuongnh;
			}
			...
			//alert(acuongnh);
			actb_curr.value = a;
			actb_cuongnhhdn.value = acuongnh;
		}
		//alert(actb_cuongnhhdn.value);
		actb_mouse_on_list = 0;
		actb_removedisp();
	}
	function actb_penter(){
		...
		for (var i=0;i<=actb_self.actb_keywords.length;i++){
			...
			if (c == actb_pos && single == '0'){
				...
				cuongnhid = actb_self.actb_cuongnhlistvalue[i] + ',';
				break;
			}
		...
		//alert(cuongnhid);
		actb_insertword(word,cuongnhid);
		l = getCaretStart(actb_curr);	
		if (actb_self.fEnter!=""){
			//cuongnh-----------
			//actb_self.actb_cuongnhvalue = actb_self.actb_cuongnhvalue + word;
			//alert(actb_self.actb_cuongnhvalue);
			//---------------
	...
	function actb_tocomplete(kc){
		...
		if (actb_self.actb_delimiter.length > 0){
			...
			//alert(actb_delimchar);
			c = 0;
			for (i=0,j=actb_cuongnhhdn.value.length;i<actb_cuongnhhdn.value.length;i++,j--){
				if (actb_cuongnhhdn.value.substr(i,j).search(',') == 0){
					c++;
					actb_delimcuongnh[c] = '';
				}else{
					if(actb_cuongnhhdn.value.charAt(i)!= ''){
						actb_delimcuongnh[c] += actb_cuongnhhdn.value.charAt(i);	
					}
				}
			}
			//Xoa id
			//alert(actb_delimcuongnh);
			var actb_idlist = '';
			for (i=0;i<actb_delimwords.length;i++){
				if(actb_delimwords[i] == ''){
					actb_delimcuongnh[i]='';
				}else{
					if(actb_delimcuongnh[i]!=''){
						actb_idlist = actb_idlist + actb_delimcuongnh[i] + ',';	
					}
				}
			}
			actb_cuongnhhdn.value = actb_idlist;
			//alert(actb_delimcuongnh);
			var l = 0;
			actb_cdelimword = -1;
			for (i=0;i<actb_delimwords.length;i++){
