...
/**
* ham nay tinh toan de tinh ra duong dan tuyet doi cua anh thong qua tham so $_GET["imgDir"]
**/
function htmlRelative($path1, $path2= '' ){
	...
	if(substr($path1, 0, 1) == "/")// neu tim thay ky tu / dau tien thi loai bo
	{
		$path1 = substr($path1 , 1, strlen($path1)-1);
	}
	
	if(substr($path1, strlen($path1)-1, 1) == "/")// neu tim thay ky tu / cuoi cung thi loai bo
	{
		$path1 = substr($path1 , 0, strlen($path1)-1);
	}
	
	if(substr($path2, 0, 1) == "/")// neu tim thay ky tu / dau tien thi loai bo
	{
		$path2 = substr($path2 , 1, strlen($path2)-1);
	}
	if(substr($path2, strlen($path2)-1, 1) == "/")// neu tim thay ky tu / cuoi cung thi loai bo
	{
	...
	$imageFolder =  htmlRelative($ImageDirectory);
	...
	if($_GET["ToDo"] == "UploadImage")
	{
		...
		{		
			...
			{
				$uploadSuccess = @copy($newFileLocation, $DOCUMENT_ROOT . $ImageDirectory . "/" . strtolower($newFileName));
		...
		{		
			...
			{
				$uploadSuccess = @copy($newFileLocation1, $DOCUMENT_ROOT . $ImageDirectory . "/" . strtolower($newFileName1));
		...
		{		
			...
			{
				$uploadSuccess = @copy($newFileLocation2, $DOCUMENT_ROOT . $ImageDirectory . "/" . strtolower($newFileName2));
		...
		{		
			...
			{
				$uploadSuccess = @copy($newFileLocation3, $DOCUMENT_ROOT . $ImageDirectory . "/" . strtolower($newFileName3));
		...
		{		
			...
			{
				$uploadSuccess = @copy($newFileLocation4, $DOCUMENT_ROOT . $ImageDirectory . "/" . strtolower($newFileName4));
	...
	?>
		...
		*/
		?>
		var imageFolder = "<?php echo $imageFolder; ?>/"
		var previousImage
		var selectedImage
		...
		<span class="appInside1" style="width:350px">
			...
			<?php if($du != "1") { ?>
				<div class="appInside3" style="padding:11px"><span class="appTitle"><?php echo sTxtUploadImage; ?></span>
					<br>
					<input type="file" name="upload" class="Text230"> <input type="submit" value="<?php echo sTxtUpload; ?>" class="Text75"> <input type="button" value="�" class="Text15" onClick="toggleUploadDiv()" id="toggleButton">
		...
		<span class="appInside1" style="width:350px">
			<div class="appInside2">
				<div class="appInside3" style="padding:11px"><span class="appTitle"><?php echo sTxtInternalImage; ?></span>
			...
			while(false !== ($file = readdir($dirHandle)))
			{
				...
				?>
					<td width="25%">
						...
						<img src="<?php echo $ImageDirectory . "/" . $file; ?>" width="80" height="80" border=1 name="<?php echo $file; ?>" name2="<?php echo urlencode($file); ?>"></div>
					</td>
				<?php
				}
	...
	?>
		...
		<span class="appInside1" style="width:350px; position:absolute">
			<div class="appInside2">
				<div class="appInside3" style="padding:11px"><span class="appTitle"><?php echo sTxtPreview; ?></span><br>
					...
					</span><input type="button" name="previewButton" value="<?php echo sTxtPreview; ?>" class="Text75" onClick="javascript:viewImage(selectedImage)" disabled=true style="position:absolute; left:257px;">
				</div>
			</div>
		</span>
		...
		<?php if($dd != "1") { ?>
		<input type="button" name="deleteButton" value="<?php echo sTxtImageDel; ?>" class="Text75" onClick="javascript:deleteImage(selectedImageEncoded)" disabled>
		<?php } ?>
		</div>
