...
// Dinh nghia duong dan den thu vien cua Zend
	...
	//echo $sRecordArchiveId;
	//Lay tu dong so den VB 
	$arrresult = $objRecordArchive->DocRecordArchivesDocRelateCheck($sRecordArchiveId);
	//var_dump($arrresult);
	$html = '<input type = "hidden" id = "checkvalue" value="';
	for($index = 0;$index < sizeof($arrresult);$index++)
		$html = $html.$arrresult[$index]['FK_DOC'].',';
	$html = $html.'" />';
	echo $html;
?>