...
* Nguoi tao: Thainv
* Ngay tao: 06/01/2009
* Y nghia:Tao lop modListType dan xuat (extends) tu lop Sys_DB_Connection
*/

class Listxml_modListType extends Sys_DB_Connection {
	/**
	 * Nguoi tao: Thainv
	 * Ngay tao: 06/01/2009
	...
	public function getAllListType($piStatus,$psTypeName,$psOwnerCode){							
		$sql = "Exec SysLib_ListtypeGetAll '" . $piStatus . "','" . $psTypeName . "','" . $psOwnerCode . "'";			
		//echo $sql . '<br>';
		$arrTempResult = $this->adodbQueryDataInNameMode($sql);			
		//var_dump($arrTempResult);
		//Xu ly ket qua
	...
	*/	
	public function updateListType($piListTypeId,$psListTypeCode,$psListTypeName,$piListTypeOrder,$psListTypeXml,$pbListTypeStatus,$psListTypeOwnerCodeList){
		// Bien luu trang thai
		$Result = null;		
		$sql = "Exec SysLib_ListtypeUpdate " . $piListTypeId . ",'".$psListTypeCode."'";
	...
	public  function deleteListType($psListTypeIdList){
		// Bien luu trang thai
		$Result = null;		
		$sql = "Exec SysLib_ListtypeDelete '" . $psListTypeIdList . "'";		
		// thuc hien cap nhat du lieu vao csdl
		try {			
			$arrTempResult = $this->adodbExecSqlString($sql) ; 			
			$Result= $arrTempResult['RET_ERROR'];
	...
	public function getSingleListType($piListTypeId){
		$sql = "Exec SysLib_ListtypeGetSingle " . $piListTypeId ;
		//echo $sql . "<br>";
		try {
			$arrTempResult = $this->adodbQueryDataInNameMode($sql);	
			//Xu ly ket qua
