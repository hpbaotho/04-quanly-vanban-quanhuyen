...
class Listxml_ListTypeController extends  Zend_Controller_Action {
	public function init(){
		...
		$this->view->LoadAllFileJsCss = $objPublicLibrary->_getAllFileJavaScriptCss('public/js/ListType','','','','js') . $objPublicLibrary->_getAllFileJavaScriptCss('public/js/ListType','','','','css').Sys_Publib_Library::_getAllFileJavaScriptCss('','js','jquery-1.4.2.min.js',',','js');
	...
	public function indexAction(){
		...
		$this->view->bodyTitle = 'DANH SÁCH LOẠI DANH MỤC';			
	...
	public function addAction(){
		...
		if($this->_request->isPost()){	
			...
			if($isUpdate == 'OK' & $isFlag=true ){
				...
				$objSysLibrary = new Sys_Library();
				
				// lay cac tham so cua form				
				$iListTypeId =(int)$filter->filter($arrInput['hdn_listtype_id']);					
				
				$sListTypeCode	=$objSysLibrary->_restoreXmlBadChar($filter->filter($arrInput['C_CODE'])) ;				
				$sListTypeName = $objSysLibrary->_restoreXmlBadChar($filter->filter($arrInput['C_NAME'])) ;			
				...
				// Lay don vi su dung
				$sListTypeOwnerCodeList = $_SESSION['OWNER_CODE'];							
	...
	public function editAction(){
		...
		$arrTempResult = array(
			...
			'C_CODE'=> $objSysLibrary->_replaceBadChar($arrResult[0]['C_CODE']),
			'C_NAME' => $objSysLibrary->_replaceBadChar($arrResult[0]['C_NAME']),
	   ...
	   if($isUpdate == 'OK'){	
	   		// lay cac tham so cua form					
			$sListTypeCode	= $objSysLibrary->_restoreXmlBadChar($this->_request->getParam('C_CODE')) ;				
			$sListTypeName = $objSysLibrary->_restoreXmlBadChar($this->_request->getParam('C_NAME'));
