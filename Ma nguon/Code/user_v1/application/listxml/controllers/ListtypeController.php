...
class Listxml_ListTypeController extends  Zend_Controller_Action {
	public function init(){
		...
		$this->view->LoadAllFileJsCss = $objPublicLibrary->_getAllFileJavaScriptCss('public/js/ListType','','','','js') . $objPublicLibrary->_getAllFileJavaScriptCss('public/js/ListType','','','','css').Sys_Publib_Library::_getAllFileJavaScriptCss('','js','jquery-1.4.3.min.js',',','js');
	...
	public function addAction(){
		...
		if($this->_request->isPost()){	
			...
			if($isUpdate == 'OK' & $isFlag=true ){
				// Tao doi tuong trong thu vien Sys
				$objSysLibrary = new Sys_Library();
				
				// lay cac tham so cua form				
				$iListTypeId =(int)$filter->filter($arrInput['hdn_listtype_id']);					
				
				$sListTypeCode	=trim($objSysLibrary->_restoreXmlBadChar($filter->filter($arrInput['C_CODE']))) ;				
				$sListTypeName = trim($objSysLibrary->_restoreXmlBadChar($filter->filter($arrInput['C_NAME']))) ;			
	...
	public function editAction(){
		...
		$arrTempResult = array(
			...
			'C_CODE'=> trim($objSysLibrary->_replaceBadChar($arrResult[0]['C_CODE'])),
			'C_NAME' => trim($objSysLibrary->_replaceBadChar($arrResult[0]['C_NAME'])),
